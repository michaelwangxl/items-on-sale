...
public class RecommendationService {
    ...
    public Recommendation getRecommendations(Integer userId) {
        ...
        //my Orders List
        List<UserOrder> myOrders = userOrderServiceRepository.findUserOrdersByUserId(userId); //get orders based on userId.
        myOrders.sort(Comparator.comparing(UserOrder::getRating).reversed());                 //sorting based on rating.(5->1)

        List<Product> myOrderList = myOrders
                .stream()
        ...
        //Wish List
        List<UserFavorite> myFav = userFavoriteRepository.findUserFavoriteByUserId(userId);   //get favorites based userId.
        List<Product> wishList = myFav
                .stream()
        ...
        //Hot Deals
        List<UserOrder> othersOrders = userOrderServiceRepository.findUserOrdersByUserIdIsNot(userId); //get others' orders except userId.
        List<Product> hotDeals = othersOrders
                .stream()
    ...
    public void updateOnSaleProducts(){
        ...
        productList.forEach(p -> {p.setOnSale(new Random().nextBoolean());productRepository.save(p);});
    }
}

...
public class RecommendationService {
	...
	public Recommendation getRecommendations(Integer userId) {
		...
		// my Orders List
		List<UserOrder> myOrders = userOrderServiceRepository.findUserOrdersByUserId(userId); // get orders based on
																								// userId.
		myOrders.sort(Comparator.comparing(UserOrder::getRating).reversed()); // sorting based on rating.(5->1)

		List<Product> myOrderList = myOrders.stream().map(o -> productRepository.findByProductId(o.getProductId())) // map
																													// to
																													// products
		...
		// Wish List
		List<UserFavorite> myFav = userFavoriteRepository.findUserFavoriteByUserId(userId); // get favorites based
																							// userId.
		List<Product> wishList = myFav.stream().map(f -> productRepository.findByProductId(f.getProductId())) // map to
																												// products
				.filter(Product::isOnSale) // filter out only on-sale ones.
				.collect(Collectors.toList());

		// Hot Deals
		List<UserOrder> othersOrders = userOrderServiceRepository.findUserOrdersByUserIdIsNot(userId); // get others'
																										// orders except
																										// userId.
		List<Product> hotDeals = othersOrders.stream().filter(o -> o.getRating() == 5) // get highest rated products
				.map(o -> productRepository.findByProductId(o.getProductId())) // map to products
				.filter(Product::isOnSale) // filter out only on-sale ones.
				.collect(Collectors.toList());

		return new Recommendation(myOrderList, wishList, hotDeals); // put 3 categories to recommendation.
	}
